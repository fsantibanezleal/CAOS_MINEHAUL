...
\newcommand{\versiondoi}{10.5281/zenodo.22834659}
\newcommand{\conceptdoi}{10.5281/zenodo.21512291}
\newcommand{\docrepo}{github.com/fsantibanezleal/CAOS\_MINEHAUL}
...
CAOS open-research programme \quad$\cdot$\quad \texttt{minehaulsim}, discrete-event simulation of mine haulage\\[2pt]
...
\thanks{Package \texttt{minehaulsim} on PyPI; source (MIT) at
...
{Santib\'a\~nez-Leal: minehaulsim, a deterministic discrete-event simulator of mine haulage on a constrained road network}
...
(shortest-queue $3{,}766$ vs nearest-shovel $2{,}874$ tonnes per hour at a matched fleet). Generated mines are
synthetic and equipment curves are class-representative envelopes, so no result predicts a specific operation
without calibration to its data. Every number and figure regenerates from the simulation outputs archived with
the source code.
\end{abstract}
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig-network.pdf}
...
vehicle weight; a retarder envelope caps the descending (empty, down-grade) speed~\cite{cat}. Travel time on a route is the
...
pure function of its \texttt{(spec, policy, seed)} and is byte-identical across operating systems and repeated runs.
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.8\textwidth]{fig-throughput.pdf}
...
to $0.70$, the classical match-factor tradeoff~\cite{morgan}. \emph{Congestion is real}: the constrained-network run trails the
...
underground mines (declines with passing bays, drift zones, ore passes, shaft bins), each accepted only when it
passes named validity checks, so every seed yields a structurally different valid mine. Every run exports interoperable artifacts: a
...
from minehaulsim.des.dispatch import (
    MinQueuePolicy)
spec = generate_open_pit(seed=42)
res  = spec.run(policy=MinQueuePolicy(), seed=7,
                until_s=6*3600)
...
The package documentation states the scope of the model on an explicit ``what it is and is not'' page. Equipment
curves are class-representative envelopes generated from first principles around public spec-sheet magnitudes, not
a manufacturer's performance table; cycle times behave like the machine class but must not be used for
procurement. The material is a single homogeneous tonnes stream: there is no ore grade, blending, dilution or
stockpile rehandle, and no fuel, tyre or emissions accounting in the present implementation. Generated mines are
synthetic, \emph{structure-real at best and always labelled} in the provenance; nothing here predicts a specific
operation without calibration to its data. The dispatch policies are transparent research baselines, not a
commercial fleet-management system; the optimisation tier lives in consumers, not here. The present implementation
also makes three simplifications: truck breakdowns materialise at cycle boundaries, surface roads are modelled flat
between graded ramps, and the underground shaft hoist is a continuous drain rather than a skip cycle. The
documentation asks a user to read the scope page first.

\section{Conclusion}

\texttt{minehaulsim} gives the open-source community what it lacked: a deterministic discrete-event simulator of
mine haulage on a constrained road network, where travel time follows grade, congestion emerges from
...
{\raggedright
The demonstration is \path{manuscripts/haulage-des/figures/haulage_sim.py}: it generates the seed-42 pit,
sweeps the truck fleet under shortest-queue dispatch in both congested and free-flow modes, compares the dispatch
policies at a matched fleet, and exports the road network, writing \path{data/haulage_sim.json} and
\path{data/network.json}. The figure script reads those artifacts; the haul-cycle schematic is a hand-authored
theme-aware drawing. Because every run is a pure function of \texttt{(spec, policy, seed)}, re-running reproduces
every number in Section~\ref{sec:results} and Figs.~\ref{fig:network}--\ref{fig:throughput} exactly.\par}
